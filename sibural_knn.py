...
def simulate_sibural_knn(agent_num, map_type, explored_data, k, monte_num, aw, dpw,init_position):
    # class advanced_knn:
            ...
            for ag in range(agent_num):
                ...
                if len(new_candidate_node) != 0:
                    ...
                    for w in range(len(path_length_list)):
                        weight = path_length_list[w] * aw + new_candidate_node_dp[w] * dpw
                        agent_final_candidate.append(new_candidate_node[w])
                        k_next_frontier_node.append([new_candidate_node[w], ag, new_candidate_node_dp[w], path_length_list[w], weight])
            ...
            for i, ag in enumerate(null_agent):
                ...
                if len(non_selected_agent_path_length) == 0:
                    k_next_frontier_node.append([[agent_list[null_agent[i]].get_position()[0],
                                                  agent_list[null_agent[i]].get_position()[1]], null_agent[i], 0, 0, 9999])
                else:
                    min_non_path_length_list = min(non_selected_agent_path_length)
                    for w in range(len(non_selected_agent_path_length)):
                        weight = non_selected_agent_path_length[w] * aw + non_selected_agent_node_dp[w] * dpw
                        agent_final_candidate.append(non_selected_agent_node[w])
                        k_next_frontier_node.append([non_selected_agent_node[w], null_agent[i],non_selected_agent_node_dp[w], non_selected_agent_path_length[w], weight])

            #print("k= " + str(k) + "일때의 다음 노드 최종 후보군들: " + str(k_next_frontier_node))



            k_next_frontier_node.sort(key=lambda k_next_frontier_node: (k_next_frontier_node[1],k_next_frontier_node[4]))
            print("k= " + str(k) + "일때의 다음 노드 최종 후보군들: ")
