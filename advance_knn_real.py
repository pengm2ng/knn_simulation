...
def simulate_advanced_knn_real(agent_num, map_type, explored_data, k, monte_num):
    # class advanced_knn:
    ...
    for monte in range(1, monte_num + 1):
        ...
        while flag:
            ...
            for k_num in range(2, k + 1):
                ...
                if len(training_labels) < k_num:
                    allocation = knn_function.allocate_frontier_node(iter_cnt, training_points, training_labels,
                                                                     candidate_node_list)
                else:
                    if len(candidate_node_list) != 0:
                        allocation = knn_function.allocate_frontier_node(k_num, training_points, training_labels,
                                                                         candidate_node_list)
                    ...
                    if len(new_candidate_node) != 0:
                        ...
                        for w in range(len(path_length_list)):
                            if path_length_list[w] == min_path:
                                ...
                                k_next_frontier_node.append([new_candidate_node[w], ag, new_candidate_node_dp[w], path_length_list[w]])
                ...
                for i, ag in enumerate(null_agent):
                    ...
                    if len(non_selected_agent_path_length) == 0:
                        k_next_frontier_node.append([[agent_list[null_agent[i]].get_position()[0],
                                                      agent_list[null_agent[i]].get_position()[1]], null_agent[i], 0, 0])
                    else:
                        ...
                        for w in range(len(non_selected_agent_path_length)):
                            if non_selected_agent_path_length[w] == min_non_path_length_list:
                                ...
                                k_next_frontier_node.append([non_selected_agent_node[w], null_agent[i],non_selected_agent_node_dp[w], non_selected_agent_path_length[w]])
                ...
                print("k= " + str(k_num) + "일때의 다음 노드 최종 후보군들: " + str(k_next_frontier_node))
                agent_num = 0
                ...
                final_node_list.append([k_num, final_node])

            print("k에 따른 최종 노드")
            for e in final_node_list:
                print(e)

            whole_length_list = []
            for i,v in enumerate(final_node_list):
                temp_length = 0
                temp_dp = 0
                for ag in range(agent_num):
                    temp_length = temp_length + v[1][ag][3]
                    temp_dp = temp_dp + v[1][ag][2]
                whole_length_list.append([v[0], temp_length, temp_dp, i])
            whole_length_list.sort(key=lambda whole_length_list: (whole_length_list[1], whole_length_list[2]))
            print(whole_length_list)
            final_k = whole_length_list[0][3]

            print(final_k)

            for ag in range(agent_num):
                frontier_function.set_explored_passnode(agent_list[ag].get_position(), changed_map)
                agent_list[ag].set_position(final_node_list[final_k][1][ag][0][0], final_node_list[final_k][1][ag][0][1], final_node_list[final_k][1][ag][3])
                print(final_node_list[final_k][1][ag][0][0], final_node_list[final_k][1][ag][0][1])
    ...
    f = open("C:/Users/장인호/Desktop/knn_simulation/real531.txt", 'a')
