...
def simulate_voronoi(agent_num, map_type, explored_data,k,monte_num,init_position):
    ...
    for monte in range(1, monte_num + 1):
        ...
        while flag:
            ...
            for w in range(len(candidate_points)):
                voronoi_map[candidate_points[w][0]][candidate_points[w][1]] = allocation[w]
            '''
            ...
            print("선택받지못한"+ str(non_selected))
            if len(non_selected) != 0:
                for ag in range(agent_num):
                    if len(path_length_list[ag]) == 1:
                        min = 9999999

                        for w in range(len(non_selected)):

                            start = agent_list[ag].get_position()
                            end = (non_selected[w][0], non_selected[w][1])
                            ...
                            if str(type(path)) != "<class 'NoneType'>":
                                if min > len(path)-1:
                                    ...
                                    min = len(path) - 1
                                    agent_path = len(path) - 1
                        path_length_list[ag].append(node)
                        path_length_list[ag].append(agent_path)
                        path_length_list[ag].append(0)
                        print(node)
                        print(non_selected)
                        non_selected.remove(node)
            else:
                for ag in range(agent_num):
                    if len(path_length_list[ag]) == 1:
                        path_length_list[ag].append([agent_list[ag].get_position()[0],agent_list[ag].get_position()[1]])
                        path_length_list[ag].append(0)
                        path_length_list[ag].append(0)
            ...
            print("2차 후보"+ str(path_length_list))
    ...
    f = open("C:/Users/장인호/Desktop/knn_simulation/result/voronoi.txt", 'a')
